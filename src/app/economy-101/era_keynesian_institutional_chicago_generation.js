export const theories = [
  {
    id: "keynesian",
    name: "케인즈주의",
    period: "1936 ~ 1970년",
    era: "케인즈·제도·시카고 세대",
    status: "20세기 중반 주류",
    scholars: [
      {
        nameKr: "존 메이너드 케인즈",
        nameEn: "John Maynard Keynes",
        achievements: "『고용, 이자 및 화폐에 관한 일반이론』, 유효수요이론",
      },
      {
        nameKr: "존 힉스",
        nameEn: "John Hicks",
        achievements: "IS-LM 모형 개발",
        nobelPrize: 1972,
        image: "/static/images/scholars/john_hicks.jpg",
      },
      {
        nameKr: "앨빈 핸슨",
        nameEn: "Alvin Hansen",
        achievements: "케인즈주의 확산, 장기 불황 이론",
      },
    ],
    keyConcepts: [
      {
        name: "유효수요이론",
        description:
          "실제 구매력으로 뒷받침되는 수요가 국민소득과 고용을 결정한다는 이론",
      },
      {
        name: "승수효과",
        description:
          "정부 지출이나 투자가 그 몇 배의 소득 증대 효과를 가져오는 현상",
      },
      {
        name: "IS-LM 모형",
        description:
          "생산물 시장의 균형(IS: Investment-Saving)과 화폐 시장의 균형(LM: Liquidity preference-Money supply)을 동시에 분석하여 국민소득과 이자율이 결정되는 과정을 설명하는 거시경제 모형",
      },
      {
        name: "재정정책",
        description: "정부 지출이나 조세를 통해 경제에 영향을 미치는 정책",
      },
      {
        name: "정부개입주의",
        description:
          "경제 안정과 성장을 위해 정부가 적극적으로 시장에 개입해야 한다는 주의",
      },
    ],
    summary:
      "1929년 대공황으로 인해 시장의 자율 조정 능력에 대한 믿음이 흔들리던 시기, 신고전학파의 자유방임주의를 비판하며 등장했습니다. 존 메이너드 케인즈는 『고용, 이자 및 화폐에 관한 일반이론』에서 경제 전체의 총수요, 즉 유효수요가 국민소득과 고용 수준을 결정한다고 주장했습니다. 그는 시장이 항상 완전고용을 달성하는 것은 아니며, 특히 불황 시에는 정부가 재정지출 확대나 감세와 같은 적극적인 재정정책을 통해 유효수요를 창출하여 경제를 안정시켜야 한다고 역설했습니다. 정부 지출이 그 몇 배의 소득 증대 효과를 가져오는 승수효과 개념도 제시했습니다. 존 힉스는 케인즈의 이론을 수학적으로 정형화한 IS-LM 모형을 개발하여 케인즈주의 분석의 주요 도구로 만들었습니다. 케인즈주의는 제2차 세계대전 이후 약 30년간 서구 자본주의 국가들의 경제정책의 이론적 기반(케인즈 혁명)이 되었습니다.",
    quote:
      '시장은 스스로 완전고용을 달성하지 못한다! 특히 불황기에는 정부가 적극적으로 돈을 풀어 총수요를 늘려야 경제를 살릴 수 있다. "장기적으로 우리는 모두 죽는다"는 것을 기억하라.',
    diagramClass: "keynesian",
    emergenceContext:
      "1929년 대공황으로 인해 기존 고전학파 및 신고전학파 경제학이 설명하지 못하는 대규모 실업과 장기 불황이 발생했습니다. 시장의 자동적인 균형 회복 능력에 대한 믿음이 흔들리면서, 정부의 적극적인 개입을 통해 유효수요를 창출하고 경제를 안정시켜야 한다는 주장이 힘을 얻었습니다.",
  },
  {
    id: "institutional",
    name: "제도주의",
    period: "1899 ~ 현재",
    era: "케인즈·제도·시카고 세대",
    status: "지속적 영향 (신제도학파로 발전)",
    scholars: [
      {
        nameKr: "소스타인 베블런",
        nameEn: "Thorstein Veblen",
        achievements: "『유한계급론』, 과시적 소비",
      },
      {
        nameKr: "존 로저스 코먼스",
        nameEn: "John Rogers Commons",
        achievements: "법과 집단행동 연구",
      },
      {
        nameKr: "웨슬리 클레어 미첼",
        nameEn: "Wesley Clair Mitchell",
        achievements: "경기변동 경험적 연구",
      },
      {
        nameKr: "로널드 코즈",
        nameEn: "Ronald Coase",
        achievements: "거래비용, 코즈 정리 (신제도학파)",
        nobelPrize: 1991,
        image: "/static/images/scholars/ronald_coase.jpg",
      },
    ],
    keyConcepts: [
      { name: "제도", description: "사회의 공식적·비공식적 규칙, 규범, 관습" },
      { name: "관습", description: "오랫동안 되풀이되어 온 사회적 행동 양식" },
      { name: "법", description: "국가에 의해 강제되는 사회 규범" },
      {
        name: "기술의 역할",
        description: "기술 발전이 경제와 사회에 미치는 영향",
      },
      {
        name: "진화론적 접근",
        description: "제도와 경제가 시간에 따라 변화하고 발전한다는 관점",
      },
      {
        name: "거래비용 (신제도학파)",
        description:
          "경제적 거래를 하는 데 드는 비용 (정보 탐색, 협상, 계약 이행 등)",
      },
    ],
    summary:
      "신고전학파의 추상적이고 정적인 분석에 반발하여, 경제 현상을 이해하기 위해서는 법, 관습, 사회 규범, 기술 등 구체적인 '제도'와 그 역사적 진화 과정을 중시해야 한다고 주장했습니다 (구제도학파). 소스타인 베블런은 『유한계급론』에서 과시적 소비와 같은 사회적, 심리적 요인이 경제 행동에 미치는 영향을 분석했습니다. 존 로저스 코먼스는 법률과 집단행동이 경제에 미치는 영향을 연구했습니다. 웨슬리 클레어 미첼은 경기변동에 대한 경험적 연구를 강조했습니다. 이후 로널드 코즈, 올리버 윌리엄슨, 더글러스 노스 등에 의해 신고전학파의 분석틀을 일부 수용하면서도 제도의 중요성을 강조하는 신제도학파로 발전했습니다. 특히 코즈는 거래비용의 개념을 통해 재산권 설정과 법 제도가 자원 배분에 미치는 영향을 분석했습니다.",
    quote:
      "경제는 진공상태에서 움직이는 것이 아니다! 법, 관습, 사회구조, 기술 같은 '제도'가 경제 주체들의 행동과 시장 결과를 결정짓는다. 경제를 이해하려면 제도의 진화를 봐야 한다.",
    diagramClass: "institutional",
    emergenceContext:
      "20세기 초, 신고전학파의 추상적이고 정적인 분석에 대한 반발로 등장했습니다. 경제 현상을 이해하기 위해서는 합리적 개인이라는 가정만으로는 부족하며, 법, 관습, 사회 규범, 기술 등 구체적인 '제도'와 그 역사적 진화 과정을 고려해야 한다는 문제의식에서 출발했습니다.",
  },
  {
    id: "chicago",
    name: "시카고학파/통화주의",
    period: "1950 ~ 1990년",
    era: "케인즈·제도·시카고 세대",
    status: "20세기 후반 주류 (신자유주의 토대)",
    scholars: [
      {
        nameKr: "밀턴 프리드먼",
        nameEn: "Milton Friedman",
        achievements: "통화량이론, 자연실업률",
        nobelPrize: 1976,
        image: "/static/images/scholars/milton_friedman.jpg",
      },
      {
        nameKr: "조지 스티글러",
        nameEn: "George Stigler",
        achievements: "정보경제학, 규제이론",
        nobelPrize: 1982,
      },
      {
        nameKr: "게리 베커",
        nameEn: "Gary Becker",
        achievements: "비시장 영역 경제학 확장",
        nobelPrize: 1992,
        image: "/static/images/scholars/gary_becker.jpg",
      },
    ],
    keyConcepts: [
      {
        name: "통화량이론",
        description: "통화 공급량의 변화가 물가 수준을 결정한다는 이론",
      },
      {
        name: "자연실업률",
        description:
          "인플레이션을 가속하지 않고 장기적으로 유지될 수 있는 실업률 수준",
      },
      {
        name: "합리적 기대가설 (일부 수용)",
        description:
          "경제 주체들이 사용 가능한 모든 정보를 활용하여 미래를 합리적으로 예측한다는 가정",
      },
      {
        name: "자유시장경제",
        description:
          "정부 개입을 최소화하고 시장의 자율적 기능에 경제 운영을 맡기는 체제",
      },
      {
        name: "작은 정부",
        description: "정부의 역할과 규모를 축소해야 한다는 주장",
      },
    ],
    summary:
      "케인즈주의의 적극적인 정부 개입 정책에 대한 비판적 입장에서 등장하여, 신고전학파의 자유시장 원리를 계승하고 발전시켰습니다. 특히 밀턴 프리드먼은 통화 공급량의 변화가 물가와 명목 소득에 장기적으로 직접적인 영향을 미친다는 통화량이론(화폐수량설 현대화)을 주장하며, 정부의 재량적 경제 정책보다는 준칙에 입각한 안정적인 통화 관리를 강조했습니다(통화주의). 그는 또한 장기적으로는 실업률이 자연실업률 수준으로 회귀하며, 정부가 이를 인위적으로 낮추려는 시도는 인플레이션만 유발한다고 주장했습니다. 조지 스티글러는 정보경제학과 규제이론에 기여했으며, 게리 베커는 경제학적 분석을 결혼, 출산, 범죄 등 비시장 영역으로 확장했습니다. 시카고학파는 1970년대 스태그플레이션 이후 케인즈주의의 대안으로 부상하며 신자유주의 정책의 이론적 토대를 제공했습니다.",
    quote:
      "인플레이션은 언제 어디서나 화폐적인 현상이다! 정부의 과도한 시장 개입과 재량적 통화정책은 경제를 불안정하게 만들 뿐, 장기적으로 통화량을 안정적으로 관리하는 것이 최선이다.",
    diagramClass: "chicago",
    emergenceContext:
      "제2차 세계대전 이후 케인즈주의가 주류 경제정책으로 자리 잡았으나, 1970년대 스태그플레이션(경기 침체 속 물가 상승) 현상이 나타나면서 케인즈주의적 처방의 한계가 드러났습니다. 이에 대한 대안으로 정부 개입 축소, 통화량 관리의 중요성, 시장 기능에 대한 신뢰를 강조하는 이론이 부상했습니다.",
    realWorldApplication: {
      title: "신자유주의 정책으로의 전환: 대처리즘과 레이거노믹스",
      story: "1970년대 스태그플레이션으로 케인즈주의적 해법이 한계를 보이자, 시카고학파의 아이디어는 새로운 정책 방향을 제시했습니다. 영국의 마거릿 대처 총리는 공기업 민영화, 규제 완화, 노동조합 약화 등 강력한 시장 중심 개혁인 '대처리즘'을 추진했습니다. 미국의 로널드 레이건 대통령 역시 감세, 규제 완화, 작은 정부를 핵심으로 하는 '레이거노믹스'를 통해 공급 중시 경제정책을 펼쳤습니다. 이들의 정책은 당시 경제에 큰 영향을 미쳤으며, 이후 신자유주의적 경제 질서 확산의 중요한 계기가 되었습니다.",
      figures: [
        { nameKr: "마거릿 대처", nameEn: "Margaret Thatcher", role: "영국 총리" },
        { nameKr: "로널드 레이건", nameEn: "Ronald Reagan", role: "미국 대통령" },
      ]
    }
  },
];
